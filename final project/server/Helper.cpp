...
std::vector<byte> Helper::toBinarySerialize(MessageType m, const std::vector<unsigned char>& message)
{
	std::vector<unsigned char> result;
	result.push_back(static_cast<byte>(m));
	for (int i = BYTE_SIZE * 3; i >= 0; i -= BYTE_SIZE)
	{
		result.push_back((message.size() >> i) & HEXADECIMAL_255);
	}
...
int Helper::getIntPartFromSocket(const SOCKET sc, const int bytesNum)
{
	if (bytesNum == 1)
	{
		return getPartFromSocket(sc, bytesNum, 0).c_str()[0];
	}
